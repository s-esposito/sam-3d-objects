    ...
    def __call__(self, inputs, target):
        ...
        for i in range(50):
            ...
            if self.black:
                ...
                if (
                    (self.grid_transform(cornergrid, transmat0, gridsize=[float(h), float(w)]).abs() > 1).sum()
                    + (self.grid_transform(cornergrid, transmat1, gridsize=[float(h), float(w)]).abs() > 1).sum()
                ) == 0:
                    break
        if i == 49:
        ...
        inputs_0 = F.grid_sample(torch.Tensor(inputs[0]).permute(2, 0, 1)[np.newaxis], vgrid[np.newaxis])[0].permute(
            1, 2, 0
        )
        if self.order == 0:
            ...
            target_0 = F.grid_sample(torch.Tensor(target).permute(2, 0, 1)[np.newaxis], vgrid[np.newaxis])[0].permute(
                1, 2, 0
            )
        ...
        if self.order == 0:
            ...
            mask_0 = F.grid_sample(torch.Tensor(mask_0).permute(2, 0, 1)[np.newaxis], vgrid[np.newaxis])[0].permute(
                1, 2, 0
            )
        mask_0[torch.isnan(mask_0)] = 0

        vgrid = self.grid_transform(meshgrid, transmat1, gridsize=[float(h), float(w)])
        inputs_1 = F.grid_sample(torch.Tensor(inputs[1]).permute(2, 0, 1)[np.newaxis], vgrid[np.newaxis])[0].permute(
            1, 2, 0
        )
...
class pseudoPCAAug(object):
    ...
    def __call__(self, inputs, target):
        inputs[0] = np.asarray(self.augcolor(Image.fromarray(np.uint8(inputs[0] * 255)))) / 255.0
        inputs[1] = np.asarray(self.augcolor(Image.fromarray(np.uint8(inputs[1] * 255)))) / 255.0
        return inputs, target
    ...
    def pca_image(self, rgb):
        ...
        for c in range(3):
            if max_abs_eig[c] > 1e-2:
                ...
                eig[:, :, c] = np.power(np.abs(eig[:, :, c]), self.pow_nomean[c]) * ((eig[:, :, c] > 0) - 0.5) * 2
        ...
        if max_abs_eig[0] > 1e-2:
            eig[:, :, 0] = np.power(np.abs(eig[:, :, 0]), self.pow_withmean[0]) * ((eig[:, :, 0] > 0) - 0.5) * 2
