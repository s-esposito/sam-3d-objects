...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Frame index to process (0-based)",
    )
...
def main():
    ...
    image_path = os.path.join(paths['frames_path'], paths['image_names'][args.frame_index])
    mask_path = os.path.join(paths['masks_path'], paths['mask_names'][args.frame_index])
    ...
    if args.first_object_only:
        masks = masks[:1]
        print(f"--first-object-only: Processing only 1 mask")
    ...
    cache_parts = [args.scene_name, f"f{args.frame_index}"]
    if args.first_object_only:
    ...
    # Initialize inference pipeline if needed
    inference = None
    if need_inference or args.use_moge:
        TAG = "hf"
    ...
    # # Save each object individually for debugging
    ...
    #     print(f"Saved individual object {i+1} Gaussian scene to {obj_ply_path}")
